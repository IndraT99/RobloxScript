...
--     warn("This script must only be executed from the official IndraHub loader")
--         return
--     end
...
-- _G.__userKey = nil
print("IndraHub Blox Fruits loading...")
...
local WindUI = loadstring(fetch("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua", "IndraHub_BloxFruit_WindUI.lua"))();
if env.IndraHubBloxFruitWindow then pcall(function() env.IndraHubBloxFruitWindow:Destroy(); end); end;
local function notify(title, content, icon)
	if WindUI and type(WindUI.Notify) == "function" then
		pcall(function() WindUI:Notify({Title = title, Content = content, Icon = icon or "info", Duration = 3}); end);
	else
		print("[IndraHub BloxFruit] " .. tostring(title) .. ": " .. tostring(content));
	end;
end;
...
env.IndraHubBloxFruitWindow = Window;
Window:SetToggleKey(Enum.KeyCode.RightControl);
Window:EditOpenButton({Title = "IndraHub", Icon = "flame", Draggable = true});
local function wrapTab(tab)
	local wrapper = {};
	function wrapper:Seperator(text) if tab.Section then return tab:Section({Title = tostring(text or "Section"), Icon = "minus"}); end; end;
	function wrapper:Line() if tab.Section then return tab:Section({Title = "-", Icon = "minus"}); end; end;
	function wrapper:Label(text)
		local label = {value = tostring(text or "")};
		if tab.Section then pcall(function() tab:Section({Title = label.value, Icon = "info"}); end); end;
		function label:Set(value) self.value = tostring(value or ""); end;
		return label;
	end;
	function wrapper:Button(title, callback)
		return tab:Button({Title = tostring(title or "Button"), Callback = callback or function() end});
	end;
	function wrapper:Toggle(title, default, desc, callback)
		return tab:Toggle({Title = tostring(title or "Toggle"), Desc = type(desc) == "string" and desc or nil, Value = default == true, Callback = callback or function() end});
	end;
	function wrapper:Dropdown(title, values, default, callback)
		return tab:Dropdown({Title = tostring(title or "Dropdown"), Values = values or {}, Value = default, Callback = callback or function() end});
	end;
	function wrapper:Slider(title, min, max, default, callback)
		return tab:Slider({Title = tostring(title or "Slider"), Value = {Min = min or 0, Max = max or 100, Default = default or min or 0}, Step = 1, Callback = callback or function() end});
	end;
	function wrapper:Textbox(title, clearOnFocus, callback)
		return tab:Input({Title = tostring(title or "Input"), Value = "", Placeholder = tostring(title or "Input"), Callback = callback or function() end});
	end;
	return wrapper;
end;
local Update = {LoadAnimation = function() return false; end, StartLoad = function() end, Loaded = function() end, SaveSettings = function() return true; end};
local Library = {Tab = function(_, title) return wrapTab(Window:Tab({Title = tostring(title or "Tab"), Icon = "circle"})); end};
local MainTab = Library:Tab("Main");
local FarmTab = Library:Tab("Farming");
local ItemsTab = Library:Tab("Items");
local SettingsTab = Library:Tab("Setting");
local LocalPlayerTab = Library:Tab("Local Player");
local HoldTab = Library:Tab("Hold Skill");
if World3 then SeaTab = Library:Tab("Sea Event"); SettingSeaTab = Library:Tab("Setting Sea"); end;
if World2 or World3 then SeaStackTab = Library:Tab("Stack Sea"); end;
if World3 then CraftTab = Library:Tab("Crafts"); DragonDojoTab = Library:Tab("Dragon Dojo"); end;
local StatsTab = Library:Tab("Stats Weapon");
if World3 or World2 then RaceV4Tab = Library:Tab("Race V4"); end;
local CombatTab = Library:Tab("Combat");
if World2 or World3 then RaidTab = Library:Tab("Raid"); end;
local EspTab = Library:Tab("Esp");
local TeleportTab = Library:Tab("Teleport");
local ShopTab = Library:Tab("Shop");
local FruitTab = Library:Tab("Devil Fruit");
local MiscTab = Library:Tab("Misc");
local ServTab = Library:Tab("Server");
notify("IndraHub", "Blox Fruits loaded. Toggle UI: RightControl", "flame");
_G.Settings = {
	Main = {
		["Select Weapon"] = "Melee",
		["Farm Mode"] = "Normal",
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("IndraHub") then
			makefolder("IndraHub");
		end;
		if not isfolder("IndraHub/Blox Fruits/") then
			makefolder("IndraHub/Blox Fruits/");
		end;
		if not isfile(("IndraHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("IndraHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("IndraHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
	...
	if Update:SaveSettings() then
		if readfile and writefile and isfile and isfolder then
			if not isfile(("IndraHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
				...
				writefile("IndraHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
			end;
		else
			return warn("Status : Undetected Executor");
		end;
