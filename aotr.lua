...
local IndraHubEnv = (getgenv and getgenv()) or _G
local IndraHubSession = tostring(os.clock()) .. "_" .. tostring(math.random(1000, 9999))

local function SetIndraHubGlobal(key, value)
	rawset(_G, key, value)
	if IndraHubEnv ~= _G then IndraHubEnv[key] = value end
end

local function GetIndraHubGlobal(key)
	local value = rawget(_G, key)
	if value ~= nil then return value end
	return IndraHubEnv[key]
end

SetIndraHubGlobal("IndraHubAOTRRunning", true)
SetIndraHubGlobal("IndraHubAOTRSession", IndraHubSession)
SetIndraHubGlobal("IndraHubAOTRLastHeartbeat", os.clock())
SetIndraHubGlobal("IndraHubAOTRError", nil)

local function IndraHubAOTRActive()
	return GetIndraHubGlobal("IndraHubAOTRRunning") == true and GetIndraHubGlobal("IndraHubAOTRSession") == IndraHubSession
end

task.spawn(function()
	while IndraHubAOTRActive() do
		SetIndraHubGlobal("IndraHubAOTRLastHeartbeat", os.clock())
		task.wait(2)
	end
end)
		...
		if webhook and webhook ~= "" then
			local payload = {
				...
				embeds = {{
					...
					footer = {
						text = "IndraHub • " .. DateTime.now():FormatLocalTime("LTS", "en-us")
					},
...
local function setupAutoExecute()
	if getgenv().AutoExecute and not getgenv().AutoExec then
		...
		queue_on_teleport([[
			...
			task.wait(5)
			getgenv().AutoExec = false
			loadstring(game:HttpGet("https://raw.githubusercontent.com/IndraT99/RobloxScript/refs/heads/main/aotr.lua"))()
		]])
	end
end
		...
		if familyRarity == "mythical" and webhook and webhook ~= "" then
			...
			local payload = {
				...
				embeds = {{
					...
					footer = { text = "IndraHub • " .. DateTime.now():FormatLocalTime("LTS", "en-us") },
					timestamp = DateTime.now():ToIsoDate()
				}}
			}
...
local function FetchIndraHubSource(url, cacheName)
...
local WindUI = loadstring(FetchIndraHubSource("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua", "IndraHub_AOTR_WindUI.lua"))()
if not WindUI then
	SetIndraHubGlobal("IndraHubAOTRError", "WINDUI FAIL")
	warn("[IndraHub AOTR] WindUI load failed")
	return
end

local Library = { Options = {}, Toggles = {}, Unloaded = false, _unloadCallbacks = {}, _window = nil }
local ThemeManager = {}
local SaveManager = {}
...
local function NewOption(id, value)
	local option = { Value = value, _callbacks = {} }
	function option:OnChanged(callback) table.insert(self._callbacks, callback) end
	function option:_fire(value)
		self.Value = value
		for _, callback in ipairs(self._callbacks) do pcall(callback, value) end
	end
	function option:SetValue(value)
		self.Value = value
		if self.Control then
			pcall(function()
				if self.Control.Set then self.Control:Set(value)
				elseif self.Control.SetValue then self.Control:SetValue(value) end
			end)
		end
		self:_fire(value)
	end
	function option:SetValues(values)
		...
		if self.Control and self.Control.Refresh then pcall(function() self.Control:Refresh(self.Values) end) end
	end
	function option:SetVisible(visible)
		self.Visible = visible
		if self.Control and self.Control.SetVisible then pcall(function() self.Control:SetVisible(visible) end) end
	end
	Options[id] = option
	return option
end
...
local function DropdownDefault(config)
	local values = config.Values or {}
	if config.Multi then return type(config.Default) == "table" and config.Default or {} end
	if type(config.Default) == "number" then return values[config.Default] end
	return config.Default or values[1]
end
...
local function WrapGroup(tab)
	...
	function group:AddToggle(id, config)
		...
		toggle.Control = WrapControl(tab:Toggle({
			Title = config.Text or config.Title or id,
			Desc = config.Tooltip or config.Description or "",
			Value = toggle.Value,
	...
	function group:AddSlider(id, config)
		...
		local step = config.Rounding == 0 and 1 or (1 / (10 ^ (config.Rounding or 1)))
		option.Control = WrapControl(tab:Slider({
			Title = config.Text or config.Title or id,
			Desc = config.Tooltip or config.Description or "",
			Value = { Min = config.Min or 0, Max = config.Max or 100, Default = option.Value },
			Step = step,
			Callback = function(value)
				option:_fire(value)
				if config.Callback then pcall(config.Callback, value) end
	...
	function group:AddDropdown(id, config)
		local option = NewOption(id, DropdownDefault(config))
		option.Values = config.Values or {}
		option.Control = WrapControl(tab:Dropdown({
			Title = config.Text or config.Title or id,
			Desc = config.Tooltip or config.Description or "",
	...
	function group:AddInput(id, config)
		local option = NewOption(id, config.Default or "")
		option.Control = WrapControl(tab:Input({
			Title = config.Text or config.Title or id,
			Desc = config.Tooltip or config.Placeholder or "",
			Value = option.Value,
	...
	function group:AddButton(config, callback)
		if type(config) == "table" then
			return tab:Button({ Title = config.Text or config.Title or "Button", Desc = config.Tooltip or config.Description or "", Callback = config.Func or config.Callback or function() end })
		end
	...
	function group:AddLabel(text)
		...
		if tab.Paragraph then label.Control = tab:Paragraph({ Title = label.Text, Desc = "" })
		elseif tab.Section then tab:Section({ Title = label.Text, Icon = "info" }) end
		function label:SetText(value)
			self.Text = tostring(value or "")
			pcall(function()
				if self.Control and self.Control.SetTitle then self.Control:SetTitle(self.Text)
				elseif self.Control and self.Control.SetDesc then self.Control:SetDesc(self.Text) end
			end)
		end
		function label:AddKeyPicker(id, config)
			local keyName = config.Default or "RightControl"
			local option = NewOption(id, Enum.KeyCode[keyName] or keyName)
			option.Key = option.Value
			option.Mode = config.Mode or "Toggle"
			...
			if tab.Keybind then option.Control = WrapControl(tab:Keybind({ Title = config.Text or id, Value = option.Key, Callback = function(value) option.Key = value option:_fire(value) end })) end
			return option
		end
		function label:AddColorPicker(id, config)
			local option = NewOption(id, config.Default or Color3.fromRGB(255, 255, 255))
			local colorpicker = tab.Colorpicker or tab.ColorPicker
			if colorpicker then option.Control = WrapControl(colorpicker(tab, { Title = config.Text or id, Value = option.Value, Callback = function(value) option:_fire(value) end })) end
			return option
		end
		return label
	end
...
function Library:CreateWindow(config)
	if IndraHubEnv.IndraHubAOTRWindow then pcall(function() IndraHubEnv.IndraHubAOTRWindow:Destroy() end) end
	local window = WindUI:CreateWindow({
		Title = "IndraHub",
		Icon = "swords",
		Author = "AOT:R | FREE",
		Folder = "IndraHub/AOTR",
	...
	IndraHubEnv.IndraHubAOTRWindow = window
	self._window = window
	...
	function wrapped:SetCornerRadius() end
	return wrapped
end

function Library:Notify(message, duration)
	local title = type(message) == "table" and (message.Title or "IndraHub") or "IndraHub"
...
function Library:Toggle(state)
	if self._window then pcall(function() self._window.Visible = state ~= false end) end
end

function Library:OnUnload(callback) table.insert(self._unloadCallbacks, callback) end
function Library:Unload()
	self.Unloaded = true
	SetIndraHubGlobal("IndraHubAOTRRunning", false)
...
function SaveManager:BuildConfigSection(tab) if tab and tab.AddLeftGroupbox then tab:AddLeftGroupbox("Config", "save"):AddLabel("WindUI config compatibility mode") end end
...
CombatGroup:AddButton({
	...
	Func = function()
		...
		if not refillPart then
			Library:Notify({ Title = "IndraHub", Description = "Refill station not found!", Time = 3 })
			return
		end
		local root = lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
		if not root then
			Library:Notify({ Title = "IndraHub", Description = "Character not loaded yet!", Time = 3 })
			return
		end
		root.CFrame = refillPart.CFrame * CFrame.new(0, 5, 10)
		Library:Notify({ Title = "IndraHub", Description = "Teleported to refill station!", Time = 2 })
	end,
...
Toggles.AutoRollToggle:OnChanged(function()
	...
	if getgenv().AutoRoll then
		if game.PlaceId ~= 13379208636 then
			Library:Notify({ Title = "IndraHub", Description = "You must be in the lobby to use family roll features.", Time = 3 })
			return
		end
		task.spawn(function()
...
Options.SelectFamily:OnChanged(function()
	if Options.SelectFamily.Value ~= "" then
		Library:Notify({ Title = "IndraHub", Description = "Families selected: " .. Options.SelectFamily.Value, Time = 2 })
	end
end)
...
ThemeManager:SetFolder("IndraHub/aotr")
SaveManager:SetFolder("IndraHub/aotr")
...
task.spawn(function()
	...
	if Toggles.AutoHideToggle.Value then
		...
		Library:Notify({ Title = "IndraHub", Description = "Auto Hid GUI", Time = 2 })
	end
end)
