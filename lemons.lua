...
Env.IndraHubLemonsRunning = true
Env.IndraHubLemonsSession = SessionId
Env.IndraHubLemonsLastHeartbeat = os.clock()
Env.IndraHubLemonsError = nil

local function IsRunning()
    return Env.IndraHubLemonsRunning and Env.IndraHubLemonsSession == SessionId
end
...
local WindUI = loadstring(Fetch("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua", "IndraHub_Lemons_WindUI.lua"))()

if not WindUI then
    warn("[ERROR]: Failed to load WindUI.")
    return
end

if Env.IndraHubLemonsWindow then pcall(function() Env.IndraHubLemonsWindow:Destroy() end) end

local Window = WindUI:CreateWindow({
    Title = "IndraHub",
    Icon = "lemon",
    Author = "Sell Lemons",
    Folder = "IndraHubLemons",
...
Env.IndraHubLemonsWindow = Window
Window:SetToggleKey(Enum.KeyCode.RightShift)
Window:EditOpenButton({ Title = "IndraHub", Icon = "lemon", Draggable = true })
    ...
    if WindUI and type(WindUI.Notify) == "function" then
        pcall(function()
            WindUI:Notify({
                ...
                Icon = data.Image or data.Icon or "info",
                Duration = data.Duration or 3,
            })
        end)
    else
        print("[IndraHub Lemons] " .. tostring(data.Title) .. ": " .. tostring(data.Content))
    end
end

function Rayfield:Destroy()
    Env.IndraHubLemonsRunning = false
    if Env.IndraHubLemonsWindow then pcall(function() Env.IndraHubLemonsWindow:Destroy() end) end
end
...
local function WrapTab(tab)
    ...
    function wrapped:CreateSection(name)
        if tab.Section then tab:Section({ Title = name, Icon = "list" }) end
    end
    ...
    function wrapped:CreateToggle(options)
        return tab:Toggle({
            Title = options.Name,
    ...
    function wrapped:CreateInput(options)
        return tab:Input({
            Title = options.Name,
            Desc = options.PlaceholderText or "",
            Value = options.CurrentValue or "",
            Placeholder = options.PlaceholderText or "",
            Callback = options.Callback,
        })
    end

    function wrapped:CreateButton(options)
        return tab:Button({
            Title = options.Name,
            Desc = options.Desc or "",
            Callback = options.Callback,
...
function Window:CreateTab(name)
    return WrapTab(Window:Tab({ Title = name, Icon = "lemon" }))
end
...
task.spawn(function() -- auto rebirth loop
    ...
    while true do task.wait(0.1)
        ...
        if tick() - LastAutoRebirthToggle < 3 then break end
        if tick() - LastSuccessfulRebirth < RebirthCooldown then break end
        ...
        if MaxRebirths > 0 then
            ...
            if CurrentRebirths >= MaxRebirths then
                break -- stop rebirthing once we hit the limit
            end
        end
        -- ===========================================
...
task.spawn(function() -- auto buy powers loop
    ...
    while true do task.wait(0.5)
        if ScriptData.AutoBuyPowers then
            TryBuyPowers()
        end
    end
end)
...
task.spawn(function() -- auto wake income loop
    ...
    while true do task.wait()
        if ScriptData.AutoWakeIncomeSources then
            TryWakeIncome()
        end
    end
end)
