...
env.IndraHubChameleonRunning = true
env.IndraHubChameleonLastHeartbeat = os.clock()
env.IndraHubChameleonError = nil

task.spawn(function()
    while env.IndraHubChameleonRunning and IsRunning do
        env.IndraHubChameleonLastHeartbeat = os.clock()
        task.wait(2)
    end
end)
...
local function SafeCall(func, ...)
    ...
    if not success then
        warn("[Chameleon] Error: " .. tostring(result))
    end
...
local function SetupGUI()
    if env.IndraHubChameleonWindow then
        pcall(function() env.IndraHubChameleonWindow:Destroy() end)
    end
    ...
    env.IndraHubChameleonWindow = Window
    ...
    SettingsTab:Section({ Title = "IndraHub Chameleon", Icon = "search" })
    ...
    SettingsTab:Button({
        ...
        Callback = function()
            ...
            env.IndraHubChameleonWindow = nil
            print("[IndraHub Chameleon] Script destroyed.")
        end,
    })
...
local function Init()
    print("[IndraHub Chameleon] Script loaded!")
    print("[IndraHub Chameleon] Press RightShift to toggle GUI")
