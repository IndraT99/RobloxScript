...
print("[IndraHub] Steal An Egg - loading...")
...
if acOk then
	print("[IndraHub] Client AC bypassed (" .. tostring(acInfo) .. " tables)")
else
	warn("[IndraHub] Client AC bypass skipped: " .. tostring(acInfo))
end
...
if not okWindUI or type(WindUI) ~= "table" then
    warn("[IndraHub] WindUI failed: " .. tostring(WindUI))
...
if okJunkie and type(Junkie) == "table" then
    ...
    warn("[IndraHub] Junkie failed: " .. tostring(Junkie))
    genv.SV_SAE_RUNNING = nil
    return
end
...
if not EggCmds or not PlotCmds or not Guard or not Lookup then
	warn("[IndraHub] Steal An Egg modules unavailable")
	genv.SV_SAE_RUNNING = nil
	return
end
...
local function farmTick()
	if State.busy then
		if State.busySince and os.clock() - State.busySince > 90 then
			State.busy = false
			State.busySince = nil
			warn("[IndraHub] Autofarm busy timeout - reset")
		else
			return
		end
	end
...
local Window = WindUI:CreateWindow({
    Title = "IndraHub - Steal An Egg",
    Icon = "swords",
    Author = "Auto Farm & Utilities",
    Folder = "IndraHubStealAnEgg",
...
track(RunService.Heartbeat:Connect(function()
	...
	if _G then
		_G.IndraHubStealAnEggLastHeartbeat = os.clock()
		_G.IndraHubStealAnEggRunning = true
	end
	if State.speedOn or State.fly then
		freezeSpeedPower()
	end
...
genv.SV_SAE_SHUTDOWN = function()
	...
	if _G then
		_G.IndraHubStealAnEggRunning = nil
	end
end
