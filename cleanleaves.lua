...
env.IndraHubCleanLeavesRunning = true
env.IndraHubCleanLeavesLastHeartbeat = os.clock()
env.IndraHubCleanLeavesError = nil
...
local Window = WindUI:CreateWindow({
    Title = "IndraHub - Clean Leaves",
    Icon = "rbxassetid://18657887261",
    Author = "IndraHub",
    Folder = "IndraHub_CleanLeaves",
...
local Tabs = {
    ...
    Lobby = Window:Tab({ Title = "Lobby", Icon = "home" }),
    Status = Window:Tab({ Title = "Status", Icon = "bar-chart" }),
}

-- Farm Tab
Tabs.Farm:Section({ Title = "FARM LOOP" })
Tabs.Farm:Toggle({
    Title = "AUTO Farm",
    Desc = "Collect, sell, buy bag - the whole loop",
    Default = CONFIG.auto,
    Callback = function(v)
        CONFIG.auto = v
        saveAuto()
...
Tabs.Farm:Toggle({
    Title = "Auto Sell / Deposit",
    Desc = "Empty bag at the richest dumpster your zone allows",
    Default = CONFIG.autoDeposit,
    Callback = function(v) CONFIG.autoDeposit = v end
})
Tabs.Farm:Toggle({
    Title = "Smooth Movement",
    Desc = "Glide between piles instead of snapping",
    Default = CONFIG.smooth,
    Callback = function(v) CONFIG.smooth = v end
})
Tabs.Farm:Toggle({
    Title = "Respect Zone Locks",
    Desc = "Skip locked zones",
    Default = CONFIG.zoneAware,
    Callback = function(v)
        CONFIG.zoneAware = v
...
Tabs.Farm:Slider({
    ...
    Callback = function(v) CONFIG.collectRadius = math.floor(v) end
})
Tabs.Farm:Slider({
    Title = "Batch Size",
    Min = 20,
    Max = 200,
    Default = CONFIG.batchMax,
    Callback = function(v) CONFIG.batchMax = math.floor(v) end
})

Tabs.Farm:Section({ Title = "SPENDING & PROGRESSION" })
Tabs.Farm:Toggle({
    Title = "Buy Bag Capacity",
    Desc = "25 -> 75 -> 500 -> 1000 for 2 / 10 / 35 cash",
    Default = CONFIG.autoBag,
    Callback = function(v) CONFIG.autoBag = v end
})
Tabs.Farm:Toggle({
    Title = "Buy the Rake",
    Desc = "7.99 cash, after bag ladder is complete",
    Default = CONFIG.autoRake,
    Callback = function(v) CONFIG.autoRake = v end
})
Tabs.Farm:Toggle({
    Title = "Unlock Vents",
    Desc = "1 cash each, instant payouts",
    Default = CONFIG.autoVent,
    Callback = function(v) CONFIG.autoVent = v end
})
Tabs.Farm:Toggle({
    Title = "Buy Tool Upgrades",
    Desc = "Batch collect ignores Grasp",
    Default = CONFIG.autoUpgrade,
    Callback = function(v) CONFIG.autoUpgrade = v end
})
Tabs.Farm:Toggle({
    Title = "Do Objectives",
    Desc = "Journal, hand upgrade, garage quests",
    Default = CONFIG.autoObjective,
    Callback = function(v) CONFIG.autoObjective = v end
})
Tabs.Farm:Toggle({
    Title = "Finish the Run",
    Desc = "All zones clear -> push vent to book clear",
    Default = CONFIG.autoFinish,
    Callback = function(v) CONFIG.autoFinish = v end
})
Tabs.Farm:Toggle({
    Title = "Skip Cutscenes",
    Desc = "Presses SKIP on opening and ending scenes",
    Default = CONFIG.autoSkip,
    Callback = function(v) CONFIG.autoSkip = v end
})

Tabs.Farm:Section({ Title = "QUICK ACTIONS" })
Tabs.Farm:Button({
    Title = "Sell Now",
    Desc = "Empty backpack at dumpster immediately",
    Callback = function() task.spawn(deposit) end
})
Tabs.Farm:Button({
    Title = "Finish Run Now",
    Desc = "Trigger run completion via vent passage",
    Callback = function() task.spawn(finishRun) end
})

-- Lobby Tab
Tabs.Lobby:Section({ Title = "LOBBY AUTOMATION" })
Tabs.Lobby:Toggle({
    Title = "Claim Daily Reward",
    Desc = "Claim daily reward if available",
    Default = CONFIG.lobbyDaily,
    Callback = function(v) CONFIG.lobbyDaily = v end
})
Tabs.Lobby:Toggle({
    Title = "Claim Group Reward",
    Desc = "Claim group rewards (turns off if not member)",
    Default = CONFIG.lobbyGroup,
    Callback = function(v) CONFIG.lobbyGroup = v end
})
Tabs.Lobby:Toggle({
    Title = "Buy Lobby Upgrades",
    Desc = "Spend diamonds on Gems, Cash, Bag upgrades",
    Default = CONFIG.lobbyUpgrades,
    Callback = function(v) CONFIG.lobbyUpgrades = v end
})
Tabs.Lobby:Toggle({
    Title = "Spin for Class",
    Desc = "Spin the 40-diamond class wheel",
    Default = CONFIG.lobbyClassSpin,
    Callback = function(v) CONFIG.lobbyClassSpin = v end
})
Tabs.Lobby:Toggle({
    Title = "Auto Start Run",
    Desc = "Stand on team pad and confirm start",
    Default = CONFIG.lobbyStart,
    Callback = function(v) CONFIG.lobbyStart = v end
})

Tabs.Lobby:Section({ Title = "MATCHMAKING SETUP" })
Tabs.Lobby:Dropdown({
    Title = "Map Selection",
    Values = { "Auto", "House", "Mansion" },
    Value = CONFIG.lobbyMap,
    Callback = function(v) CONFIG.lobbyMap = v end
})
Tabs.Lobby:Dropdown({
    Title = "Difficulty",
    Values = { "Easy", "Medium", "Hard", "Impossible" },
    Value = CONFIG.lobbyDifficulty,
    Callback = function(v) CONFIG.lobbyDifficulty = v end
})
Tabs.Lobby:Button({
    Title = "Start Run Now",
    Desc = "Step onto team pad and start immediately",
    Callback = function() task.spawn(startRun) end
})

Tabs.Lobby:Section({ Title = "ACCOUNT OVERVIEW" })
local lobbyLabel1 = Tabs.Lobby:Label({ Title = "Diamonds: - | Class: -" })
local lobbyLabel2 = Tabs.Lobby:Label({ Title = "Upgrades: Gems - | Cash - | Bag -" })
local lobbyLabel3 = Tabs.Lobby:Label({ Title = "Phase: Ready" })

-- Status Tab
Tabs.Status:Section({ Title = "ANTI-AFK & SYSTEM" })
Tabs.Status:Toggle({
...
local liveLabel1 = Tabs.Status:Label({ Title = "Status: Initializing..." })
local liveLabel2 = Tabs.Status:Label({ Title = "Cash: $0.00 (+$0.00/s) | Deposits: 0" })
local liveLabel3 = Tabs.Status:Label({ Title = "Bag: 0 / 25" })
local liveLabel4 = Tabs.Status:Label({ Title = "Field: 0 workable | Hit rate: 0%" })
local liveLabel5 = Tabs.Status:Label({ Title = "Zone: None | Dumpster: $0.00/leaf" })
local liveLabel6 = Tabs.Status:Label({ Title = "Note: Ready" })

task.spawn(function()
    while alive() do
        pcall(function()
            if env.IndraHubCleanLeavesRunning then
                env.IndraHubCleanLeavesLastHeartbeat = os.clock()
            end

            local up = LOBBY.upgrades or {}
            lobbyLabel1:SetTitle(string.format("Diamonds: %d | Class: %s", LOBBY.diamonds or 0, tostring(LOBBY.class or "-")))
            lobbyLabel2:SetTitle(string.format("Upgrades: Gems %s | Cash %s | Bag %s", tostring(up.Gems or 0), tostring(up.Cash or 0), tostring(up.BagCapacity or 0)))
            lobbyLabel3:SetTitle("Lobby Phase: " .. tostring(LOBBY.phase or "idle") .. (LOBBY.note ~= "" and (" - " .. LOBBY.note) or ""))

            if not IN_RUN then
                liveLabel1:SetTitle("Status: In Lobby (" .. tostring(LOBBY.phase or "idle") .. ")")
                return
            end

            refreshStatus()
            liveLabel1:SetTitle(string.format("Status: RUNNING (%s) | Cleared: %d / %d", tostring(STATE.phase), STATE.cleared, STATE.field))
            liveLabel2:SetTitle(string.format("Cash: $%.2f (+$%.3f/s) | Deposits: %d", STATE.cash, STATE.rate, STATE.deposits))
            liveLabel3:SetTitle(string.format("Bag: %d / %d", STATE.leaves, STATE.cap))
            liveLabel4:SetTitle(string.format("Field: %d workable | Hit rate: %d%% at r%d", STATE.workable, math.floor(STATE.hitRate * 100), STATE.radius))
            local d = activeDump()
            liveLabel5:SetTitle(string.format("Zone: %s | Dumpster: $%.2f/leaf", tostring(STATE.zone), d and d.price or 0))
            liveLabel6:SetTitle("Note: " .. tostring(STATE.note))
        end)
        task.wait(0.5)
    end
end)
