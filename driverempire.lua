...
local okWind, WindUI = pcall(function()
    ...
    return loadstring(loadRemoteSource(url, "IndraHub_DrivingEmpire_WindUI.lua"))()
end)
if not okWind or type(WindUI) ~= "table" then warn("[IndraHub Driving Empire] WindUI load failed:", WindUI); return end

local GuiLibrary = {}
local WIND_ICONS = { success = "circle-check", error = "circle-x", warning = "triangle-alert", info = "info" }

function GuiLibrary:CreateNotification(title, msg, dur, kind)
    local payload = {
        Title = title,
        Content = msg,
        Icon = WIND_ICONS[kind] or WIND_ICONS.info,
        Duration = dur or 3,
    }
    pcall(WindUI.Notify, WindUI, payload)
end

function GuiLibrary:CreateWindow(_, size)
    local oldWindow = env.IndraHubDrivingEmpireWindow
    if oldWindow then pcall(oldWindow.Destroy, oldWindow) end

    local options = {
        Title = "IndraHub",
        Icon = "car",
        Author = "Driving Empire",
        Folder = "IndraHubDrivingEmpire",
        Size = size or UDim2.fromOffset(560, 420),
        Transparent = true,
        Theme = "Dark",
        Resizable = true,
        SideBarWidth = 160,
    }
    local window = WindUI:CreateWindow(options)
    env.IndraHubDrivingEmpireWindow = window
    pcall(window.SetToggleKey, window, Enum.KeyCode.RightControl)
    pcall(window.EditOpenButton, window, { Title = "IndraHub", Icon = "car", Draggable = true })
    return window
end

function GuiLibrary:CreateTab(window, name)
    return window:Tab({ Title = name, Icon = "circle" })
end

function GuiLibrary:CreateSection(tab, title)
    local data = { Title = title, Content = "" }
    if tab.Section then return tab:Section({ Title = title, Icon = "minus" }) end
    if tab.Paragraph then return tab:Paragraph(data) end
    if tab.AddParagraph then return tab:AddParagraph(data) end
end

function GuiLibrary:CreateToggle(tab, title, default, callback)
    return tab:Toggle({ Title = title, Value = default == true, Callback = callback })
end

function GuiLibrary:CreateInput(tab, title, callback)
    return tab:Input({ Title = title, Value = "", Placeholder = title, Callback = callback })
end

function GuiLibrary:CreateSlider(tab, title, min, max, default, callback)
    local step = (max <= 5 or default % 1 ~= 0) and 0.1 or 1
    return tab:Slider({ Title = title, Value = { Min = min, Max = max, Default = default }, Step = step, Callback = callback })
end

function GuiLibrary:CreateButton(tab, title, callback)
    return tab:Button({ Title = title, Callback = callback })
end

function GuiLibrary:CreateLabel(tab, text)
    local label
    local blankParagraph = { Title = text, Content = "" }
    if tab.Paragraph then label = tab:Paragraph(blankParagraph)
    elseif tab.AddParagraph then label = tab:AddParagraph(blankParagraph)
    else label = tab:Section({ Title = text }) end
    return {
        SetText = function(v)
            local update = { Title = v, Content = "" }
            pcall(function()
                if label.SetTitle then label:SetTitle(v)
                elseif label.Set then label:Set(update)
                elseif label.Update then label:Update(update) end
            end)
        end,
...
function GuiLibrary:CreateDropdown(tab, title, values, callback)
    local dd = tab:Dropdown({ Title = title, Values = values, Value = values[1], Callback = callback })
    dd.UpdateOptions = function(newValues, selectFirst)
        pcall(function()
            if dd.Refresh then dd:Refresh(newValues, selectFirst)
            elseif dd.Set then dd:Set({ Values = newValues, Value = selectFirst and newValues[1] or nil }) end
        end)
    end
    return dd
end
...
local function invokeSharedRemote(name, ...)
    if Remotes and Remotes.invokeServer then
        local ok, a, b = pcall(Remotes.invokeServer, name, ...)
        if ok then return a, b end
    end
    return invokeRemoteFunction(name, ...)
end

local function notify(title, msg, dur, kind)
    GuiLibrary:CreateNotification(title, msg, dur or 3, kind or "info")
end
...
local function pivotPos(inst)
    local ok, pv = pcall(inst.GetPivot, inst)
...
local function countNamedTools(container, toolName)
...
local function getOwnedToolCount(toolName)
    return countNamedTools(LocalPlayer.Character, toolName)
        + countNamedTools(LocalPlayer:FindFirstChild("Backpack"), toolName)
end
...
local function playerHasAnyBag()
    ...
    return getOwnedToolCount("CriminalMoneyBag") > 0
end
...
local function countMoneyBags()
    ...
    return getOwnedToolCount("CriminalMoneyBag")
end
local function checkDropoffRequirements()
    if (LocalPlayer:GetAttribute("JobId") or "") ~= "Criminal" then return false, "Not on the Criminal job" end
    if not playerHasAnyBag() then return false, "No money bags to deposit" end
    if getStars() < 5 then return false, ("Need 5 stars (have %d)"):format(getStars()) end
    if hasArrestDebounce() then return false, "Arrest debounce active" end
    return true
end

local function startJobSession(jobId)
    pcall(function()
        ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("RequestStartJobSession"):FireServer(jobId, "jobPad")
    end)
    if Remotes and Remotes.fireServer then
        pcall(Remotes.fireServer, "RequestStartJobSession", jobId, "jobPad")
    end
end
...
task.spawn(function()
    task.wait(3)
    for _, pos in ipairs(HUNT_POSITIONS) do requestStreamAround(pos); task.wait(0.5) end
end)
...
local function getAtmModel(adornee)
    ...
    while n and n ~= Workspace do
        local ok, has = pcall(CollectionService.HasTag, CollectionService, n, "CriminalATM")
...
local function getNearestATM()
    ...
    for _, e in ipairs(listAvailableATMs()) do
        local pos = pivotPos(e.model) or pivotPos(e.adornee)
        if pos then
...
if Remotes then
    pcall(function()
        ...
        Remotes.connect("HeistHazardReturnOffer", function()
            pcall(Remotes.fireServer, "HeistHazardReturnChoice", false)
        end)
    end)
end

local function computeHeistState(heistId, data)
    if not data or not data.MinStartTime or not data.HeistDuration then return false, math.huge end
...
local function isAnyHeistOpen()
    ...
    for id, data in pairs(_heistDataCache) do
        if computeHeistState(id, data) then return true, id end
    end
    return false, nil
end
...
local function pressE(times)
    if not okVIM or not VirtualInputManager then return end
...
local function getLootChildren()
    ...
    for _, heist in ipairs(heists:GetChildren()) do
        local loot = heist:FindFirstChild("HeistAsset") and heist.HeistAsset:FindFirstChild("Loot")
        if loot then
...
local AutoFarm = {
    ...
    HoldGrace = 0.4, DepositThreshold = 0,
}
...
local function bustOneATM(entry)
    ...
    if hasATMBustDebounce() and not waitForBustDebounceClear(8) then return false, "ATMBustDebounce" end

    local pPos = pivotPos(atm) or pivotPos(adornee)
    if not pPos then return false, "noPivot" end
    local function standNear()
        local h = getHRP()
        if h then h.CFrame = CFrame.new(pPos + Vector3.new(0, 3, 0)) end
    end
    standNear(); task.wait(0.1)
    ...
    if not ok or not accepted then return false, ("BustStart failed: %s"):format(tostring(reason or accepted)) end

    local hold = CriminalUtil.GetATMBustTime(LocalPlayer)
    local stopAt = tick() + math.max(0, hold - 2.5) + AutoFarm.HoldGrace
    while tick() < stopAt do
        if not AutoFarm.Enabled then return false, "userCancelled" end
        local h = getHRP()
        if h and (h.Position - pPos).Magnitude > actDist then standNear() end
        task.wait(0.05)
    end
    ...
    if not ok2 or not completed then return false, ("BustComplete failed: %s"):format(tostring(reason2 or completed)) end
    return true
end
...
local function tryDepositAtDropoff()
    ...
    for _ = 1, 3 do
        ...
        h.CFrame = outside; task.wait(0.15)
        h.CFrame = inside
        fireAllPromptsUnder(spawner)
        pressE(2)
        if firetouchinterest then pcall(firetouchinterest, h, triggerPart, 0) end
        task.wait(0.2)
        lastOk, lastAccepted, lastReason = pcall(function()
            local remoteName = CriminalConstants and CriminalConstants.RemoteFunctions
                and CriminalConstants.RemoteFunctions.AttemptCriminalJobComplete
                or "AttemptCriminalJobComplete"
            return invokeSharedRemote(remoteName, spawner)
        end)
        task.wait(0.15)
        if firetouchinterest then pcall(firetouchinterest, h, triggerPart, 1) end
        h.CFrame = outside; task.wait(0.1)
        if not playerHasAnyBag() then break end
    end
...
local AutoDeposit = { Enabled = false, Delay = 3, Running = false }
task.spawn(function()
    while true do
        task.wait(AutoDeposit.Delay)
        if AutoDeposit.Enabled and not AutoDeposit.Running and playerHasAnyBag() then
            AutoDeposit.Running = true
            local ok, err = tryDepositAtDropoff()
            if not ok then warn("[IndraHub Driving Empire] auto deposit failed:", err) end
            AutoDeposit.Running = false
        end
    end
end)
...
task.spawn(function()
    while true do
        ...
        if AntiArrest.Enabled and not inHeistNow() then
            ...
            if pct and pct >= AntiArrest.Threshold then antiArrestPanic() end
        end
    end
end)
if Remotes then
...
local function depositAndEvade(reason)
    ...
    if ok then
        ...
        warn("[IndraHub Driving Empire] deposit failed:", err)
    end
end
...
local function runHeistOnce()
    ...
    if gate then
        tp(CFrame.new(gate.Position + Vector3.new(0, 4, 0))); task.wait(0.3)
        local h = getHRP()
        if h then
...
task.spawn(function()
    while true do
        ...
        if isInSeat() then AutoFarm.Phase = "inSeat"; continue end

        if (LocalPlayer:GetAttribute("JobId") or "") ~= "Criminal" then
            startJobSession("Criminal"); task.wait(0.5); continue
        end
        ...
        if entry then
            ...
            if ok then
                ...
                if err and err:find("Debounce") then waitForBustDebounceClear(6)
        ...
        if checkDropoffRequirements() then
            depositAndEvade("All ATMs busted — depositing"); continue
        end
        if getStars() == 0 and not isWanted() and isAnyHeistOpen() then
            runHeistOnce(); continue
        end
...
local BASIC_CAR_SPAWN_PLAYER_CF = CFrame.new(1289.836, 11.044, 738.345)
local VEHICLE_REMOTE_PATH = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("VehicleEvent")
...
local function getDrivenVehicleModel()
    ...
    local current = seat
    while current do
        if current:IsA("Model") and current:FindFirstChild("Core") then return current, seat end
...
local function getWeightPart(model)
    ...
    return model:FindFirstChild("Weight") or model:FindFirstChild("WeightPart") or model.PrimaryPart
end
...
local function getVehicleId(model)
    ...
    if vo and vo:IsA("ObjectValue") and vo.Value then return vo.Value.Name end
    return nil
end

local function GetBestOwnedCar()
    return MANUAL_BEST_CAR ~= "" and MANUAL_BEST_CAR or nil
end

local function ensureBestCarSpawnedAndEntered()
    local hrp = getHRP()
    if not hrp then warn("[IndraHub Driving Empire] No HRP"); return false end
    hrp.CFrame = BASIC_CAR_SPAWN_PLAYER_CF; task.wait(0.4)

    local bestCarToSpawn = GetBestOwnedCar() or "Unknown"
    if bestCarToSpawn == "Unknown" then warn("[IndraHub Driving Empire] No cars detected"); return false end

    pcall(function()
        VEHICLE_REMOTE_PATH:FireServer("Spawn", bestCarToSpawn, nil, nil, SPAWN_DIRECTION)
    end)
    ...
    local function findNearbyVehicle(maxDist, targetCarName)
        ...
        local cands = {}
        local searchRoots = { Workspace, Workspace:FindFirstChild("Cars"),
            Workspace:FindFirstChild("Vehicles"), Workspace:FindFirstChild("SpawnedCars") }
        for _, root in ipairs(searchRoots) do
            if root then
                for _, obj in ipairs(root:GetDescendants()) do
                    if obj:IsA("Model") and obj ~= LocalPlayer.Character then
                        local seat = obj:FindFirstChildOfClass("VehicleSeat") or obj:FindFirstChildOfClass("Seat")
                        if seat and obj:FindFirstChild("Core") then
                            local dist = (obj:GetPivot().Position - playerPos).Magnitude
                            if dist < maxDist then
                                table.insert(cands, { model = obj, seat = seat, dist = dist })
                            end
                        end
                    end
                end
        ...
        table.sort(cands, function(a, b)
            local function score(e)
                local s = (e.seat.Occupant and 0 or 100)
                if targetCarName and e.model.Name:lower():find(targetCarName:lower()) then s = s + 1000 end
                return s
            end
            local sa, sb = score(a), score(b)
            return sa ~= sb and sa > sb or a.dist < b.dist
        end)
        return cands[1] and cands[1].model or nil
    end

    local bestCar
    local startTime = tick()
    while tick() - startTime < 10 and not bestCar do
        bestCar = findNearbyVehicle(220, bestCarToSpawn)
        if not bestCar then task.wait(0.5) end
    end
    if not bestCar then warn("[IndraHub Driving Empire] No spawned car found nearby"); return false end
    ...
    task.wait(0.2)
    cam.CFrame = CFrame.new(bestCar:GetPivot().Position + Vector3.new(15, 8, 15), bestCar:GetPivot().Position)
    task.wait(1.2)
...
-- Police / Security
local Police = { CrimeScene = false, Arrest = false, CSStats = 0, ArrStats = 0 }
...
local function getCrimeSceneParts()
    ...
    for _, spawner in ipairs(spawners:GetChildren()) do
        ...
        if cs and cs:GetAttribute("State") ~= "Used" then
            ...
            if eng == nil or eng == LocalPlayer.UserId then table.insert(out, cs) end
        end
    end
    return out
end
...
local function useCrimeSceneOnce(cs, pos)
    ...
    stand(); task.wait(0.15)
    local started = invokeRemoteFunction("AttemptStartUsingCrimeScene", cs)
    if not started and Remotes.invokeServer then
        local ok, accepted = pcall(Remotes.invokeServer, "AttemptStartUsingCrimeScene", cs)
        started = ok and accepted
    end
    ...
    local finished = invokeRemoteFunction("AttemptFinishUsingCrimeScene", cs)
    if not finished and Remotes.invokeServer then
        local ok, accepted = pcall(Remotes.invokeServer, "AttemptFinishUsingCrimeScene", cs)
        finished = ok and accepted
    end
    if not finished then return false, "finishRejected" end
    return true
end
...
task.spawn(function()
    while true do
        task.wait(0.3)
        if not Police.CrimeScene or isInSeat() then continue end
        if (LocalPlayer:GetAttribute("JobId") or "") ~= "Security" then
            startJobSession("Security"); task.wait(0.6); continue
        end
        ...
        for _, e in ipairs(scenes) do
            ...
            if useCrimeSceneOnce(e.cs, e.pos) then Police.CSStats = Police.CSStats + 1 end
            task.wait(0.1)
        end
    end
end)
...
local function getWantedCriminals()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            local ok, wanted = pcall(function() return CriminalUtil and CriminalUtil.IsPlayerWanted(plr) end)
...
task.spawn(function()
    while true do
        ...
        for _, crim in ipairs(getWantedCriminals()) do
            if not Police.Arrest then break end
            local cHRP = crim.Character and crim.Character:FindFirstChild("HumanoidRootPart")
            local myHRP = getHRP()
            if cHRP and myHRP then
                myHRP.CFrame = cHRP.CFrame * CFrame.new(0, 0, 4)
                fireRemoteEvent("NotifyStartArresting", crim, 0)
                if Remotes and Remotes.fireServer then pcall(Remotes.fireServer, "NotifyStartArresting", crim, 0) end
                local stop = tick() + 6
                local done = false
                while tick() < stop and Police.Arrest do
                    local ch = crim.Character and crim.Character:FindFirstChild("HumanoidRootPart")
                    local mh = getHRP()
                    if not ch or not mh then break end
                    mh.CFrame = ch.CFrame * CFrame.new(0, 0, 4)
                    fireRemoteEvent("RequestArrestCriminal", crim)
                    if Remotes and Remotes.fireServer then pcall(Remotes.fireServer, "RequestArrestCriminal", crim) end
                    if not (CriminalUtil and CriminalUtil.IsPlayerWanted(crim)) then done = true; break end
                    task.wait(0.4)
                end
                fireRemoteEvent("NotifyStopArresting", crim)
                if Remotes and Remotes.fireServer then pcall(Remotes.fireServer, "NotifyStopArresting", crim) end
...
local function getDeliveryState()
    ...
    ok, st = pcall(DeliveryJobTask.GetCurrentDeliveryState, DeliveryJobTask)
...
local function deliveryGoInteract(targetPos)
    local loc = nearestDeliveryLocationTo(targetPos)
    if not loc then return false end
    local d = math.max(0.1, AutoDelivery.Delay or 0.45)
    teleportEntity(targetPos, 5); task.wait(d)
    for _ = 1, 3 do
        pcall(Remotes.fireServer, "DeliveryLocationInteracted", loc)
        fireRemoteEvent("DeliveryLocationInteracted", loc)
        task.wait(d * 0.8)
    end
    return true, loc
end
...
task.spawn(function()
    while true do
        ...
        if (LocalPlayer:GetAttribute("JobId") or "") ~= "Delivery" then
            startJobSession("Delivery"); task.wait(1); continue
        end
        local st = getDeliveryState()
        if not st then task.wait(0.6); continue end
        local carried = st.ItemsCarried or 0

        if st.PickupPosition and carried < (st.MaxCapacity or 4) then
            local ok, loc = deliveryGoInteract(st.PickupPosition)
            if ok and loc then
                local stop = tick() + 2
                while tick() < stop and AutoDelivery.Enabled do
                    local s2 = getDeliveryState()
                    if not s2 or (s2.ItemsCarried or 0) > carried or not s2.PickupPosition then break end
                    task.wait(0.2)
                end
                pcall(Remotes.fireServer, "DeliveryLocationLeft", loc)
                fireRemoteEvent("DeliveryLocationLeft", loc)
            end
            continue
        end
        if st.DestinationPosition then
            local ok, loc = deliveryGoInteract(st.DestinationPosition)
            if ok and loc then
                local stop = tick() + 2.5
                while tick() < stop and AutoDelivery.Enabled do
                    local s2 = getDeliveryState()
...
local function claimAllPlaytime()
    for i = 1, 12 do fireRemoteEvent("PlayRewards", i, false); task.wait(0.08) end
    pcall(Remotes.fireServer, "ClaimRewards")
    pcall(Remotes.fireServer, "RaceLeaderboardClaimRewards")
end
task.spawn(function()
    while true do
...
task.spawn(function()
    while true do
        ...
        local col = Color3.fromHSV((tick() * Rainbow.Speed) % 1, 1, 1)
        for _, sec in ipairs(Rainbow.Sections) do
            pcall(invokeRemoteFunction, "CustomizationPurchase", "Paint", sec, vid, { Color = col })
        end
        task.wait(iv)
    end
end)
...
task.spawn(function()
    while true do
        ...
        if not model then continue end
        local ok, pivot = pcall(model.GetPivot, model)
        if not ok or not pivot or pivot.UpVector.Y >= 0.4 then continue end
        ...
        pcall(function()
            model:PivotTo(CFrame.lookAt(pos, pos + flatFwd))
            local wp = getWeightPart(model)
            if wp then
                wp.AssemblyLinearVelocity = Vector3.zero
                wp.AssemblyAngularVelocity = Vector3.zero
...
task.spawn(function()
    while true do
        task.wait(0.3)
        if PlayerMods.WalkEnabled then
            local hum = getHumanoid()
...
-- Car Mods
local CarMods = {
    Enabled = false, SpeedMult = 1, AccelMult = 1, BrakeMult = 1,
    GripMult = 1, TurnMult = 1, NitroMult = 1, InfNitro = false, InstantStop = false,
}
local _modTuneRef, _baseTune, _modsDirty = nil, nil, false

local function snapshotTune(t)
    local snap = {
        TransmissionSpeeds = {}, TransmissionTorque = {},
        BrakeTorque = t.BrakeTorque, RearWheelsTraction = t.RearWheelsTraction,
        FrontWheelsTraction = t.FrontWheelsTraction, TurnSpeed = t.TurnSpeed,
        TurnRadius = t.TurnRadius, NitrousForce = t.NitrousForce,
    }
...
task.spawn(function()
    while true do
        ...
        if t ~= _modTuneRef then
            _modTuneRef = t; _baseTune = snapshotTune(t); _modsDirty = false
        end
        if CarMods.Enabled then
            pcall(applyTune, t, _baseTune, CarMods); _modsDirty = true
        elseif _modsDirty then
            pcall(applyTune, t, _baseTune, STOCK_MULTS); _modsDirty = false
        end
        ...
        if CarMods.InstantStop then
            pcall(function()
                ...
                if braking then
                    local wp = getWeightPart(ctrl.VehicleModel) or getWeightPart(getDrivenVehicleModel())
                    if wp then
                        wp.AssemblyLinearVelocity = Vector3.new(0, wp.AssemblyLinearVelocity.Y, 0)
                        wp.AssemblyAngularVelocity = Vector3.zero
                    end
                end
            end)
...
local ESP = {
    ...
    Color = Color3.fromRGB(232, 65, 78), MaxDist = 2000,
}
local espObjects = {}
...
Players.PlayerRemoving:Connect(function(plr)
    ...
    if o then
        for _, d in pairs(o) do if d then pcall(d.Remove, d) end end
        espObjects[plr] = nil
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if ESP.Box and o.box then
            o.box.Size = Vector2.new(w, h); o.box.Position = Vector2.new(cx - w/2, top)
            o.box.Color = ESP.Color; o.box.Visible = true
        elseif o.box then o.box.Visible = false end

        if ESP.Name and o.name then
            o.name.Text = plr.Name; o.name.Position = Vector2.new(cx, top - 16); o.name.Visible = true
        elseif o.name then o.name.Visible = false end

        if ESP.Distance and o.dist then
            o.dist.Text = ("%dm"):format(math.floor(dist))
            o.dist.Position = Vector2.new(cx, top + h + 2); o.dist.Visible = true
        elseif o.dist then o.dist.Visible = false end

        if ESP.Tracer and o.tracer then
            local vp = Camera.ViewportSize
            o.tracer.From = Vector2.new(vp.X/2, vp.Y); o.tracer.To = Vector2.new(cx, top + h)
            o.tracer.Color = ESP.Color; o.tracer.Visible = true
        ...
        if ESP.HealthBar and o.hp and o.hpbg then
            ...
            local bx = cx - w/2 - 5
            o.hpbg.Size = Vector2.new(3, h); o.hpbg.Position = Vector2.new(bx, top); o.hpbg.Visible = true
...
-- Car Farm (Path Replay)
local CarFarm = { Enabled = false, Speed = 5, Mode = "Replay" }
...
{9876.2764,91.9203,-6935.0015,0.8542,-0.0000,-0.5199,-0.0000,1.0000,-0.0001,0.5199,0.0001,0.8542}
    ...
    for _, row in ipairs(rawData) do
        table.insert(RecordedPath, CFrame.new(
...
local PathReplay = { Index = 1, Progress = 0 }

local function startPathReplay(model)
    if not RecordedPath or #RecordedPath < 2 then return false end
    PathReplay.Index = 1; PathReplay.Progress = 0
    pcall(model.PivotTo, model, RecordedPath[1])
    local wp = getWeightPart(model)
    if wp then pcall(function()
        wp.AssemblyLinearVelocity = Vector3.zero; wp.AssemblyAngularVelocity = Vector3.zero
    end) end
    return true
end
...
task.spawn(function()
    while true do
        ...
        while PathReplay.Progress >= 1 do
            ...
            if PathReplay.Index >= N then snapToStart(); wrapped = true; break end
        ...
        if not wrapped then
            ...
            pcall(model.PivotTo, model, target)
            local wp = getWeightPart(model)
            if wp then
...
do
    local ok, w = pcall(function()
        return GuiLibrary:CreateWindow("IndraHub", UDim2.new(0, 560, 0, 420), nil)
    end)
    if not ok then warn("[IndraHub Driving Empire] CreateWindow errored:", w); return end
    window = w
end

local tabFarm = GuiLibrary:CreateTab(window, "Auto Farm")
local tabCar  = GuiLibrary:CreateTab(window, "Car")
local tabTele = GuiLibrary:CreateTab(window, "Teleports")
local tabESP  = GuiLibrary:CreateTab(window, "ESP")
local tabMisc = GuiLibrary:CreateTab(window, "Misc")

-- ATM / Heist Farm
GuiLibrary:CreateSection(tabFarm, "ATM / Heist Farm")
GuiLibrary:CreateToggle(tabFarm, "Criminal Auto Farm", false, function(s)
    AutoFarm.Enabled = s
    AutoFarm.Stats = { ATMs = 0, Heists = 0, Failed = 0, Deposits = 0 }
    notify("Farm", s and "Started" or "Stopped", 2, s and "success" or "info")
end)
GuiLibrary:CreateToggle(tabFarm, "TP up on arrest (off in heist)", false, function(s)
    AntiArrest.Enabled = s
end)
GuiLibrary:CreateInput(tabFarm, "Auto-deposit at $ (e.g. 100k, 1M)", function(text)
...
GuiLibrary:CreateSlider(tabFarm, "ATM hold grace (s)", 0, 1.5, 0.4, function(v) AutoFarm.HoldGrace = v end)
GuiLibrary:CreateButton(tabFarm, "Deposit Now", function()
    task.spawn(function()
...
GuiLibrary:CreateToggle(tabFarm, "Auto Deposit Bags", false, function(s)
    AutoDeposit.Enabled = s
    notify("Deposit", s and "Auto deposit ON" or "Auto deposit OFF", 2, s and "success" or "info")
end)

-- Security Job
GuiLibrary:CreateSection(tabFarm, "Security Job")
GuiLibrary:CreateButton(tabFarm, "Join Security Job", function()
    startJobSession("Security"); notify("Police", "Requested security job.", 2, "info")
end)
GuiLibrary:CreateToggle(tabFarm, "Auto Crime Scene", false, function(s)
    Police.CrimeScene = s; if s then Police.CSStats = 0 end
    notify("Police", s and "Crime scene farm ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateToggle(tabFarm, "Auto Arrest Criminals", false, function(s)
    Police.Arrest = s; if s then Police.ArrStats = 0 end
    notify("Police", s and "Auto-arrest ON" or "OFF", 2, s and "success" or "info")
end)

-- Delivery Job
GuiLibrary:CreateSection(tabFarm, "Delivery Job")
GuiLibrary:CreateButton(tabFarm, "Join Delivery Job", function()
    startJobSession("Delivery"); notify("Delivery", "Requested delivery job.", 2, "info")
end)
GuiLibrary:CreateLabel(tabFarm, "Spawn a car & sit in it. Drives the car to deliveries.")
GuiLibrary:CreateToggle(tabFarm, "Full Auto Delivery (Car)", false, function(s)
    AutoDelivery.Enabled = s
    notify("Delivery", s and "Full auto ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateInput(tabFarm, "Teleport delay (s) — higher = safer", function(text)
...
-- Status Labels
GuiLibrary:CreateSection(tabFarm, "Status")
local lblCrim1 = GuiLibrary:CreateLabel(tabFarm, "Idle")
local lblCrim2 = GuiLibrary:CreateLabel(tabFarm, "—")
local lblPolice = GuiLibrary:CreateLabel(tabFarm, "Idle")
local lblDelivery = GuiLibrary:CreateLabel(tabFarm, "Idle")

-- Car Tab
GuiLibrary:CreateSection(tabCar, "Car Auto Farm")
GuiLibrary:CreateLabel(tabCar, "Sit in your best car, then toggle. Drives a fixed loop.")
GuiLibrary:CreateToggle(tabCar, "Car Auto Farm", false, function(s)
    if s then
        CarFarm.Enabled = false; task.wait(0.1)
        if not RecordedPath or #RecordedPath < 2 then
            notify("Car Farm", "Path data not loaded.", 5, "error"); return
        end
        local m = getDrivenVehicleModel()
        if not m then
            notify("Car Farm", "Spawning best car...", 3, "info")
            if not pcall(ensureBestCarSpawnedAndEntered) then
                notify("Car Farm", "Auto-spawn failed.", 4, "error"); return
            end
            task.wait(0.8)
            m = getDrivenVehicleModel()
        end
        if not m then notify("Car Farm", "Sit in a car manually.", 5, "warning"); return end
        if startPathReplay(m) then
            CarFarm.Mode = "Replay"; CarFarm.Enabled = true
            notify("Car Farm", ("%d waypoints — driving"):format(#RecordedPath), 3, "success")
        else
            notify("Car Farm", "Failed to start replay", 3, "error")
        end
    else
        CarFarm.Enabled = false; CarFarm.Mode = nil
        notify("Car", "Stopped.", 2, "info")
    end
end)
GuiLibrary:CreateSlider(tabCar, "Drive Speed", 1, 20, 5, function(v) CarFarm.Speed = v end)
local lblCar = GuiLibrary:CreateLabel(tabCar, "Idle")

GuiLibrary:CreateSection(tabCar, "Car Mods")
GuiLibrary:CreateLabel(tabCar, "Sit in a car. Toggle on, then tune the multipliers.")
GuiLibrary:CreateToggle(tabCar, "Enable Car Mods", false, function(s)
    CarMods.Enabled = s
    notify("Car", s and "Car mods ON" or "OFF (stock restored)", 2, s and "success" or "info")
end)
GuiLibrary:CreateSlider(tabCar, "Top Speed x", 1, 10, 1, function(v) CarMods.SpeedMult = v end)
GuiLibrary:CreateSlider(tabCar, "Acceleration x", 1, 10, 1, function(v) CarMods.AccelMult = v end)
GuiLibrary:CreateSlider(tabCar, "Brake Force x", 1, 10, 1, function(v) CarMods.BrakeMult = v end)
GuiLibrary:CreateSlider(tabCar, "Grip x", 1, 6, 1, function(v) CarMods.GripMult = v end)
GuiLibrary:CreateSlider(tabCar, "Turn Speed x", 1, 5, 1, function(v) CarMods.TurnMult = v end)
GuiLibrary:CreateSlider(tabCar, "Nitro Force x", 1, 8, 1, function(v) CarMods.NitroMult = v end)
GuiLibrary:CreateToggle(tabCar, "Infinite Nitro", false, function(s)
    CarMods.InfNitro = s; notify("Car", s and "Infinite nitro ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateToggle(tabCar, "Anti-Flip (auto-upright)", false, function(s) AntiFlip.Enabled = s end)
GuiLibrary:CreateToggle(tabCar, "Instant Stop (brake = stop)", false, function(s)
    CarMods.InstantStop = s; notify("Car", s and "Instant stop ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateToggle(tabCar, "Rainbow Car (server)", false, function(s)
    Rainbow.Enabled = s; notify("Car", s and "Rainbow ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateSlider(tabCar, "Rainbow Speed", 0.1, 3, 0.5, function(v) Rainbow.Speed = v end)
GuiLibrary:CreateSlider(tabCar, "Rainbow Interval (s)", 0.2, 2, 0.4, function(v) Rainbow.Interval = v end)
GuiLibrary:CreateButton(tabCar, "Despawn Vehicle", function()
    pcall(function() ReplicatedStorage.Remotes.VehicleEvent:FireServer("Despawn") end)
end)
...
GuiLibrary:CreateSection(tabTele, "Teleport (car if seated, else you)")
GuiLibrary:CreateLabel(tabTele, "In a car? It teleports the car with you.")
for _, e in ipairs(TELE_LOCATIONS) do
    GuiLibrary:CreateButton(tabTele, e[1], function()
        teleportEntity(e[2], 5); notify("Teleport", "→ " .. e[1], 2, "info")
    end)
end
GuiLibrary:CreateSection(tabTele, "Players")
local teleTargetName = nil
local teleDropdown = GuiLibrary:CreateDropdown(tabTele, "Target Player", (function()
    local names = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then table.insert(names, p.Name) end
    end
    return #names > 0 and names or { "(none)" }
end)(), function(v) teleTargetName = v end)
GuiLibrary:CreateButton(tabTele, "Refresh Players", function()
    local names = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then table.insert(names, p.Name) end
    end
    if #names == 0 then names = { "(none)" } end
    if teleDropdown and teleDropdown.UpdateOptions then teleDropdown.UpdateOptions(names, true) end
end)
GuiLibrary:CreateButton(tabTele, "Teleport To Player", function()
...
-- ESP Tab
GuiLibrary:CreateSection(tabESP, "Player ESP")
GuiLibrary:CreateToggle(tabESP, "Enable ESP", false, function(s) ESP.Enabled = s end)
GuiLibrary:CreateToggle(tabESP, "Box", true, function(s) ESP.Box = s end)
GuiLibrary:CreateToggle(tabESP, "Name", true, function(s) ESP.Name = s end)
GuiLibrary:CreateToggle(tabESP, "Distance", true, function(s) ESP.Distance = s end)
GuiLibrary:CreateToggle(tabESP, "Health Bar", true, function(s) ESP.HealthBar = s end)
GuiLibrary:CreateToggle(tabESP, "Tracers", false, function(s) ESP.Tracer = s end)
GuiLibrary:CreateToggle(tabESP, "Team Check (hide allies)", false, function(s) ESP.TeamCheck = s end)
GuiLibrary:CreateSlider(tabESP, "Max Distance", 100, 5000, 2000, function(v) ESP.MaxDist = v end)

-- Misc Tab
GuiLibrary:CreateSection(tabMisc, "Auto Claim")
GuiLibrary:CreateToggle(tabMisc, "Auto Claim Rewards (playtime+pending)", false, function(s)
    PlaytimeRewards.Enabled = s; if s then task.spawn(claimAllPlaytime) end
    notify("Misc", s and "Auto-claim ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateButton(tabMisc, "Claim Playtime Now", function()
    task.spawn(claimAllPlaytime); notify("Misc", "Fired playtime claims.", 2, "info")
end)
GuiLibrary:CreateToggle(tabMisc, "Auto Collect Cash Drops", false, function(s)
    CashCollect.Enabled = s
    notify("Misc", s and "Cash drop collect ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateToggle(tabMisc, "Fast Loading (skip load screens)", false, function(s)
    FastLoad.Enabled = s; if s then pcall(killLoadingScreens) end
    notify("Misc", s and "Fast loading ON" or "OFF", 2, s and "success" or "info")
end)

GuiLibrary:CreateSection(tabMisc, "Vehicle Fly")
GuiLibrary:CreateLabel(tabMisc, "In a car: WASD move, Space up, Shift down.")
GuiLibrary:CreateToggle(tabMisc, "Vehicle Fly", false, function(s)
    VehFly.Enabled = s; notify("Misc", s and "Vehicle fly ON" or "OFF", 2, s and "success" or "info")
end)
GuiLibrary:CreateSlider(tabMisc, "Fly Speed", 20, 400, 80, function(v) VehFly.Speed = v end)

GuiLibrary:CreateSection(tabMisc, "Player")
GuiLibrary:CreateToggle(tabMisc, "Walk Speed", false, function(s)
    PlayerMods.WalkEnabled = s
    if not s then local h = getHumanoid() if h then pcall(function() h.WalkSpeed = 16 end) end end
end)
GuiLibrary:CreateSlider(tabMisc, "Walk Speed Value", 16, 200, 32, function(v) PlayerMods.WalkSpeed = v end)
...
task.spawn(function()
    while true do
        ...
        local heistTxt
        if not _heistDataCache then heistTxt = "loading"
        else
        ...
        local dtxt
        if dst then
            dtxt = (dst.ItemsCarried or 0) > 0 and ("Dropoff (carrying %d)"):format(dst.ItemsCarried) or "Pickup"
        else
...
notify("IndraHub", "Driving Empire loaded", 4, "success")
