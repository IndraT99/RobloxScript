...
local Window = WindUI:CreateWindow({
    Title = "IndraHub - FindEgg",
    Icon = "rbxassetid://18657887261",
    Author = "IndraHub",
    Folder = "IndraHub_FindEgg",
...
local Tabs = {
    ...
    Plot = Window:Tab({ Title = "Plot", Icon = "coin" }),
}

Tabs.Farm:Section({ Title = "EGG HUNT" })
Tabs.Farm:Toggle({
    Title = "Auto farm",
    Desc = "Grab the best egg in reach and carry it home",
    Default = CONFIG.autoFarm,
    Callback = function(v) CONFIG.autoFarm = v end
})
Tabs.Farm:Toggle({
    Title = "Leave the treadmill first",
    Desc = "Training silently blocks every pickup",
    Default = CONFIG.stopTreadmill,
    Callback = function(v) CONFIG.stopTreadmill = v end
})
Tabs.Farm:Toggle({
    Title = "Train when the field is exhausted",
    Desc = "Endurance opens the next bridge, and beyond it eggs are worth hundreds of times more",
    Default = CONFIG.autoTrain,
    Callback = function(v) CONFIG.autoTrain = v end
})
Tabs.Farm:Dropdown({
    Title = "Minimum rarity",
    Desc = "Ignore anything below this on the ground",
    Values = { "common", "rare", "epic", "legendary", "mythic", "secret", "og", "abyssal", "celestial", "omega" },
    Value = CONFIG.minRarity,
    Callback = function(v) CONFIG.minRarity = v end
})

local rangeInfoLabel = Tabs.Farm:Label({ Title = "The north is walled by endurance; the bridge cannot be warped past." })
local bandLabel = Tabs.Farm:Label({ Title = "Band Z: 0" })

Tabs.Plot:Section({ Title = "HATCH & PLACE" })
Tabs.Plot:Toggle({
    Title = "Hatch at the cauldron",
    Default = CONFIG.autoHatch,
    Callback = function(v) CONFIG.autoHatch = v end
})
Tabs.Plot:Slider({
    Title = "Hatch from",
    Desc = "Eggs banked before HATCH ALL fires",
    Default = CONFIG.hatchAt or 3,
    Min = 1,
    Max = 12,
    Callback = function(v) CONFIG.hatchAt = math.floor(v) end
})
Tabs.Plot:Toggle({
    Title = "Place brainrots",
    Default = CONFIG.autoPlace,
    Callback = function(v) CONFIG.autoPlace = v end
})
Tabs.Plot:Toggle({
    Title = "Swap on base value",
    Desc = "A full plot evicts its weakest BASE, not its lowest label",
    Default = CONFIG.autoSwap,
    Callback = function(v) CONFIG.autoSwap = v end
})
Tabs.Plot:Slider({
    Title = "Swap margin",
    Desc = "Higher keeps levelled seats longer",
    Default = CONFIG.swapMargin or 1.5,
    Min = 1,
    Max = 5,
    Callback = function(v) CONFIG.swapMargin = v end
})

Tabs.Plot:Section({ Title = "MONEY" })
Tabs.Plot:Toggle({
    Title = "Collect the slots",
    Desc = "Touch sweep - the collect remote does nothing",
    Default = CONFIG.autoCollect,
    Callback = function(v) CONFIG.autoCollect = v end
})
Tabs.Plot:Toggle({
    Title = "Upgrade levels",
    Desc = "About a 2.5 second payback early on",
    Default = CONFIG.autoUpgrade,
    Callback = function(v) CONFIG.autoUpgrade = v end
})
Tabs.Plot:Slider({
    Title = "Clicks per pass",
    Default = CONFIG.upgradesPass or 6,
    Min = 1,
    Max = 20,
    Callback = function(v) CONFIG.upgradesPass = math.floor(v) end
})
Tabs.Plot:Toggle({
    Title = "Auto rebirth",
    Desc = "Needs money plus N copies of one brainrot",
    Default = CONFIG.autoRebirth,
    Callback = function(v) CONFIG.autoRebirth = v end
})

Tabs.Plot:Section({ Title = "RUN STATS" })
local runLabel1 = Tabs.Plot:Label({ Title = "" })
local runLabel2 = Tabs.Plot:Label({ Title = "" })
local runLabel3 = Tabs.Plot:Label({ Title = "" })

Tabs.Plot:Section({ Title = "ANTI-AFK" })
Tabs.Plot:Toggle({
...
env.IndraHubFindEggRunning = true
env.IndraHubFindEggLastHeartbeat = os.clock()
...
task.spawn(function()
    while _G.__FINDEGG == GEN do
        pcall(function()
            if env.IndraHubFindEggRunning then
                env.IndraHubFindEggLastHeartbeat = os.clock()
            end
            
            local free, taken = slotPrompts()
            bandLabel:SetTitle(("band ends at z %d | %d eggs in reach mode %s | last egg: %s"):format(
                STATE.bandZ, STATE.eggsSeen, tostring(STATE.mode), tostring(STATE.lastEgg)
            ))
            
            runLabel1:SetTitle(("grabbed %d | banked %d | swaps %d"):format(STATE.grabbed, STATE.banked, STATE.swaps))
            runLabel2:SetTitle(("weakest seat: %s"):format(tostring(STATE.weakSeat)))
            runLabel3:SetTitle(STATE.busy and (STATE.busy .. ": " .. STATE.note) or tostring(STATE.note))
        end)
        task.wait(1)
    end
end)
...
print("[findegg] loaded - gen " .. GEN .. " with WindUI")
