...
if getGlobal("IndraHubEvomonRunning") then return end
setGlobal("IndraHubEvomonRunning", true)

task.spawn(function()
    while task.wait(2) do
        if not getGlobal("IndraHubEvomonRunning") then break end
        setGlobal("IndraHubEvomonLastHeartbeat", os.time())
    end
end)
...
local Shared = type(...) == 'table' and ... or {}
...
local function sendWebhook(embed)
    ...
    local body = HttpService:JSONEncode({ username = "IndraHub Hub", embeds = { embed } })
    task.spawn(function()
        pcall(httpRequest, {
...
WebhookBox:AddButton({
    ...
    Func = function()
        sendWebhook({
            title = "Test Webhook",
            description = "IndraHub Hub webhook is working.",
            color = 0x2ECC71,
        })
...
local AUTO_EXECUTE_SCRIPT = 'if not game:IsLoaded() then game.Loaded:Wait() end local env = (getgenv and getgenv()) or _G if env.IndraHubAutoExecuted == game.JobId then return end env.IndraHubAutoExecuted = game.JobId task.wait(3) loadstring(game:HttpGet("https://raw.githubusercontent.com/joustingmatch/IndraHub/main/loader.lua"))()'
...
local function apply3DRendering()
    ...
    if State.Disable3DRendering then
        ...
        DisableRenderingGui.Name = "IndraHubDisable3DRendering"
...
PlayerUtilityBox:AddToggle("AutoExecute", {
    Text = "Auto Execute",
    Tooltip = "Queues the IndraHub loader to run again after server hops or reconnects.",
    Default = false,
    Callback = function(value)
...
if Env.IndraHubEvomonCleanup then
    pcall(Env.IndraHubEvomonCleanup)
end

local LoadToken = {}
Env.IndraHubEvomonLoadToken = LoadToken

local function isStaleLoad()
    return Env.IndraHubEvomonLoadToken ~= LoadToken
end
...
Shared.DISCORD_LINK = "https://discord.gg/2PPBJsmqr"
Shared.WINDOW_IMAGE = "rbxassetid://18657887261"


-- ==========================================
-- WindUI ADAPTER (replaces Obsidian/LinoriaLib)
-- ==========================================
if isStaleLoad() then
...
function Library:Unload()
    ...
    pcall(function()
        if Library._indraWindow then
            Library._indraWindow:Destroy()
        end
    end)
end

local function makeToggleWrapper(flag, opts, windSection)
    local wrapper = { Value = opts.Default or false, _cbs = {} }

    function wrapper:SetValue(v)
        self.Value = v
        if self._wt and self._wt.Set then pcall(self._wt.Set, self._wt, v) end
    ...
    local ok, wt = pcall(function()
        return windSection:Toggle({
            Title = opts.Text or flag,
            Default = opts.Default or false,
            Callback = function(v)
                wrapper.Value = v
                if opts.Callback then pcall(opts.Callback, v) end
                for _, cb in ipairs(wrapper._cbs) do pcall(cb, v) end
            end,
        })
    end)
    if ok then wrapper._wt = wt end

    Library.Toggles[flag] = wrapper
    return wrapper
end

local function makeOptionWrapper(flag, opts, windSection, widgetType)
    local wrapper = { Value = opts.Default or opts.Value or "" }

    function wrapper:SetValue(v)
        self.Value = v
        if self._wo and self._wo.Set then pcall(self._wo.Set, self._wo, v) end
        if opts.Callback then pcall(opts.Callback, v) end
    end

    function wrapper:SetValues(newVals)
        self._values = newVals
        if self._wo and self._wo.Refresh then
            pcall(self._wo.Refresh, self._wo, newVals)
        end
    end

    local ok, wo
    if widgetType == "dropdown" then
        ok, wo = pcall(function()
            return windSection:Dropdown({
                Title = opts.Text or flag,
    ...
    elseif widgetType == "input" then
        ok, wo = pcall(function()
            return windSection:Input({
                Title = opts.Text or flag,
                Value = tostring(opts.Default or opts.Value or ""),
                Callback = function(v)
                    wrapper.Value = v
                    if opts.Callback then pcall(opts.Callback, v) end
                end,
    ...
    if ok then wrapper._wo = wo end

    Library.Options[flag] = wrapper
    return wrapper
end

local function makeGroupbox(windTab, name)
    local section
    pcall(function()
        section = windTab:Section({ Title = name })
    end)
    if not section then
        section = windTab
    end
    ...
    function gb:AddToggle(flag, opts)
        return makeToggleWrapper(flag, opts or {}, section)
    end

    function gb:AddDropdown(flag, opts)
        return makeOptionWrapper(flag, opts or {}, section, "dropdown")
    end

    function gb:AddInput(flag, opts)
        return makeOptionWrapper(flag, opts or {}, section, "input")
    end

    function gb:AddSlider(flag, opts)
        return makeOptionWrapper(flag, opts or {}, section, "slider")
    end

    function gb:AddButton(opts)
        pcall(function()
            section:Button({
                Title = opts.Text or "Button",
    ...
    function gb:AddLabel(text) end

    return gb
end

local function makeTab(windTab)
    local tab = {}

    function tab:AddLeftGroupbox(name, icon)
        return makeGroupbox(windTab, name)
    end
    ...
    function tab:AddLeftTabbox(name)
        local tabbox = {}
        function tabbox:AddTab(tabName, icon)
            return makeGroupbox(windTab, name .. " > " .. tabName)
        end
        return tabbox
    end
...
Library._indraWindow = indraWindow

local Tabs = {
    Pets     = makeTab(indraWindow:Tab({ Title = "Pets",     Icon = "paw-print" })),
    Farm     = makeTab(indraWindow:Tab({ Title = "Farm",     Icon = "swords" })),
    Modes    = makeTab(indraWindow:Tab({ Title = "Modes",    Icon = "layers" })),
    Player   = makeTab(indraWindow:Tab({ Title = "Player",   Icon = "user" })),
    Combat   = makeTab(indraWindow:Tab({ Title = "Combat",   Icon = "shield" })),
    Shop     = makeTab(indraWindow:Tab({ Title = "Shop",     Icon = "shopping-cart" })),
    Hatching = makeTab(indraWindow:Tab({ Title = "Hatching", Icon = "egg" })),
}
Shared.Tabs = Tabs
...
Shared.AddDiscordButton = function(groupbox)
    pcall(function()
        groupbox:AddButton({
            Text = "Join Discord (discord.gg/2PPBJsmqr)",
            Func = function()
                pcall(function()
                    if setclipboard then setclipboard("https://discord.gg/2PPBJsmqr") end
                end)
...
pcall(function()
    ...
    if not Env._IndraHubSpeedHooked then
        Env._IndraHubSpeedHooked = true
...
pcall(function()
    ...
    if not Env._IndraHubAutoSpeedHooked then
        Env._IndraHubAutoSpeedHooked = true
...
Env.IndraHubEvomonCleanup = function()
...
-- return Shared (removed for single-file execution)
...
local SKILL_ROTATIONS_FOLDER = "IndraHubHub/Evomon"
...
local function ensureSkillRotationsFolder()
    ...
    if not isfolder("IndraHubHub") then pcall(makefolder, "IndraHubHub") end
...
do
    ...
    gui.Name = "IndraHubRotationEditor"
...
local CUSTOM_TEAMS_FILE = "IndraHubEvomonCustomTeams.json"
local HttpService = game:GetService("HttpService")
local FixedPetMap = {}
    ...
    if ok and type(data) == "table" and type(data.goodsScrollList) == "table" then
        for _, row in ipairs(data.goodsScrollList) do
            if type(row) == "table" and type(row.goodsId) == "number" then
                ...
                local label = string.format("%s \u{B7} %s %s [%s]", tostring(row.name), tostring(row.priceText), currency, stockLabel)
