...
env.IndraHubAmmoClickRunning = true
env.IndraHubAmmoClickLastHeartbeat = os.clock()
env.IndraHubAmmoClickError = nil
...
local Window = WindUI:CreateWindow({
    Title = "IndraHub - Ammo Click",
    Icon = "rbxassetid://18657887261",
    Author = "IndraHub",
    Folder = "IndraHub_AmmoClick",
...
_G.__AMMOCLICK_WIN = Window

local Tabs = {
    Farm = Window:Tab({ Title = "Farm", Icon = "bolt" }),
    Progression = Window:Tab({ Title = "Progression", Icon = "shopping-cart" }),
    Status = Window:Tab({ Title = "Status", Icon = "bar-chart" }),
}

-- Farm Tab
Tabs.Farm:Section({ Title = "AUTO ENGINE" })
Tabs.Farm:Toggle({
    Title = "Auto Ammo",
    Desc = "AddKick from anywhere on map, best unlocked target",
    Default = CONFIG.autoKick,
    Callback = function(v) CONFIG.autoKick = v end
})
Tabs.Farm:Toggle({
    Title = "Auto Walls",
    Desc = "Pins character inside the current wall and shoots it",
    Default = CONFIG.autoWalls,
    Callback = function(v) CONFIG.autoWalls = v end
})
Tabs.Farm:Toggle({
    Title = "Auto Claim",
    Desc = "Claims deepest finished stage; resets the run",
    Default = CONFIG.autoClaim,
    Callback = function(v) CONFIG.autoClaim = v end
})
Tabs.Farm:Toggle({
    Title = "Auto Rebirth",
    Desc = "Free & keeps everything; fires when level bar is full",
    Default = CONFIG.autoRebirth,
    Callback = function(v) CONFIG.autoRebirth = v end
})

Tabs.Farm:Section({ Title = "TUNING & ACTIONS" })
Tabs.Farm:Slider({
    Title = "Depth Safety %",
    Min = 10,
    Max = 90,
    Default = math.floor(CONFIG.depthSafety * 100),
    Callback = function(v) CONFIG.depthSafety = v / 100 end
})
Tabs.Farm:Slider({
    Title = "Hit Gap (ms)",
    Min = 100,
    Max = 1000,
    Default = math.floor(CONFIG.hitGap * 1000),
    Callback = function(v) CONFIG.hitGap = v / 1000 end
})
Tabs.Farm:Button({
    Title = "Claim Rewards Now",
    Desc = "Claim daily reward and banked spins",
    Callback = function() task.spawn(claimFreeRewards) end
})
Tabs.Farm:Button({
    Title = "Resync Walls",
    Desc = "Resync wall data with server",
    Callback = function() call(WallService:RequestSync(), 6) end
})

-- Progression Tab
Tabs.Progression:Section({ Title = "PURCHASES & UPGRADES" })
Tabs.Progression:Toggle({
    Title = "Auto Buy Guns",
    Desc = "Free Nebula Cannon first, then best wins allow",
    Default = CONFIG.autoWeapon,
    Callback = function(v) CONFIG.autoWeapon = v end
})
Tabs.Progression:Toggle({
    Title = "Auto Buy Boosts",
    Desc = "Damage before Wins, max 20% of balance",
    Default = CONFIG.autoBoosts,
    Callback = function(v) CONFIG.autoBoosts = v end
})
Tabs.Progression:Toggle({
    Title = "Auto Open Eggs",
    Desc = "Wins-priced eggs only (skips Robux eggs)",
    Default = CONFIG.autoEggs,
    Callback = function(v) CONFIG.autoEggs = v end
})
Tabs.Progression:Toggle({
    Title = "Auto Free Rewards",
    Desc = "Daily rewards and banked wheel spins",
    Default = CONFIG.autoRewards,
    Callback = function(v) CONFIG.autoRewards = v end
})
Tabs.Progression:Toggle({
    Title = "Auto Roll Titles",
    Desc = "Free title rolls (x1.2 to x150 multiplier)",
    Default = CONFIG.autoTitles,
    Callback = function(v) CONFIG.autoTitles = v end
})

Tabs.Progression:Section({ Title = "PETS MANAGEMENT" })
Tabs.Progression:Toggle({
    Title = "Auto Equip Best Pets",
    Desc = "Equips highest multiplier pets into slots",
    Default = CONFIG.autoPets,
    Callback = function(v) CONFIG.autoPets = v end
})
Tabs.Progression:Toggle({
    Title = "Auto Delete Weak Pets",
    Desc = "Clears pets below keep limit (equipped pets safe)",
    Default = CONFIG.autoCullPets,
    Callback = function(v) CONFIG.autoCullPets = v end
})
Tabs.Progression:Slider({
    Title = "Keep Best Pets",
    Desc = "Number of strongest pets to preserve from deletion",
    Min = 5,
    Max = 200,
    Default = CONFIG.keepPets,
    Callback = function(v) CONFIG.keepPets = math.floor(v) end
})
...
local runLabel = Tabs.Status:Label({ Title = "Stage: - | Wall: 0 | Phase: idle" })
local claimsLabel = Tabs.Status:Label({ Title = "Claims: 0 | Rebirths: 0 | Pets Deleted: 0" })
local economyLabel = Tabs.Status:Label({ Title = "Ammo: 0 | Wins: 0 (+0/s)" })
local multLabel = Tabs.Status:Label({ Title = "Gun: - | Pets: 0 | Title: -" })
local levelLabel = Tabs.Status:Label({ Title = "Level: 0 / 0 | Target: -" })
local noteLabel = Tabs.Status:Label({ Title = "Note: Starting" })

task.spawn(function()
    while alive() do
        pcall(function()
            if env.IndraHubAmmoClickRunning then
                env.IndraHubAmmoClickLastHeartbeat = os.clock()
            end

            local bag, mult = bestBag()
            runLabel:SetTitle(string.format("Stage %s | Wall %d | Phase: %s", tostring(STATE.target), STATE.cursor, tostring(STATE.phase)))
            claimsLabel:SetTitle(string.format("Claims: %d | Rebirths Done: %d | Pets Deleted: %d", STATE.claims, STATE.rebirthsDone, STATE.petsDeleted))
            economyLabel:SetTitle(string.format("Ammo: %s | Wins: %s (+%s/s)", abbreviate(STATE.ammo), abbreviate(STATE.wins), abbreviate(income)))
            multLabel:SetTitle(string.format("Gun: %s | Pets: %d (x%.1f) | Title: %s (x%s)", tostring(STATE.weapon), STATE.pets, STATE.petMult, tostring(STATE.title), tostring(STATE.titleMult)))
            local rbReady = (STATE.level or 0) >= (STATE.levelMax or math.huge) and " (REBIRTH READY)" or ""
            levelLabel:SetTitle(string.format("Level: %d / %d%s | Target: %s (x%s)", STATE.level or 0, STATE.levelMax or 0, rbReady, tostring(bag), tostring(mult)))
            noteLabel:SetTitle("Note: " .. tostring(STATE.note))
        end)
        task.wait(0.5)
    end
end)
...
print("[ammoclick] running with WindUI - IndraHub")
