...
env.IndraHubCutGrassRunning = true
env.IndraHubCutGrassLastHeartbeat = os.clock()
env.IndraHubCutGrassError = nil
...
do
	...
	if re then
		re.OnClientEvent:Connect(function(payload)
			...
			if env.IndraHubCutGrassRunning then
				env.IndraHubCutGrassError = "ANTI-CHEAT WARNING: " .. tostring(reason)
			end
		end)
	end
end
...
local Window = WindUI:CreateWindow({
	Title = "IndraHub - Cut Grass Adventure",
	Icon = "rbxassetid://18657887261",
	Author = "IndraHub",
	Folder = "IndraHub_CutGrass",
...
local Tabs = {
	Farm = Window:Tab({ Title = "Farm", Icon = "bolt" }),
	Upgrades = Window:Tab({ Title = "Upgrades", Icon = "shopping-cart" }),
	Status = Window:Tab({ Title = "Status", Icon = "bar-chart" }),
}

-- Farm Tab
Tabs.Farm:Section({ Title = "LOOP & STRENGTH" })
Tabs.Farm:Toggle({
	Title = "AUTO Farm",
	Desc = "Click, train, cut, collect, sell, repeat",
	Default = CONFIG.auto,
	Callback = function(v)
		CONFIG.auto = v
...
Tabs.Farm:Toggle({
	Title = "Auto Click Strength",
	Desc = "12.5 strength/s paced click loop",
	Default = CONFIG.click,
	Callback = function(v) CONFIG.click = v end
})
Tabs.Farm:Toggle({
	Title = "Auto Train on Pad",
	Desc = "Stacks with clicking: 2.75/s alone, 13.37/s together",
	Default = CONFIG.train,
	Callback = function(v) CONFIG.train = v end
})
Tabs.Farm:Slider({
	Title = "Train Secs / Trip",
	Min = 0,
	Max = 90,
	Default = CONFIG.trainSecs,
	Callback = function(v) CONFIG.trainSecs = math.floor(v) end
})
Tabs.Farm:Slider({
	Title = "Min Pad Multiplier",
	Min = 1,
	Max = 15,
	Default = CONFIG.trainMinMult,
	Callback = function(v) CONFIG.trainMinMult = math.floor(v) end
})

Tabs.Farm:Section({ Title = "CUTTING & LOOT" })
Tabs.Farm:Toggle({
	Title = "Cut Grass",
	Desc = "Opens the next zone - permanent progress",
	Default = CONFIG.cut,
	Callback = function(v) CONFIG.cut = v end
})
Tabs.Farm:Toggle({
	Title = "Pick Up Loot",
	Desc = "The only income - ranked on sell price",
	Default = CONFIG.loot,
	Callback = function(v) CONFIG.loot = v end
})
Tabs.Farm:Slider({
	Title = "Loot Secs / Lap",
	Min = 15,
	...
	Callback = function(v) CONFIG.tripSecs = math.floor(v) end
})
Tabs.Farm:Slider({
	Title = "Cut Secs / Lap",
	Min = 10,
	...
	Callback = function(v) CONFIG.cutSecs = math.floor(v) end
})

Tabs.Farm:Section({ Title = "ZONE PROGRESSION" })
Tabs.Farm:Dropdown({
	Title = "Zone Mode",
	Values = { "Auto", "Manual" },
...
Tabs.Farm:Slider({
	Title = "Manual Zone Target",
	Desc = "Only used while Zone Mode is Manual",
	Min = 1,
	Max = 13,
	Default = CONFIG.zoneTarget,
	Callback = function(v) CONFIG.zoneTarget = math.floor(v) end
})
Tabs.Farm:Slider({
	Title = "Hold Walled Zone (s)",
	Desc = "Wait time before probing walled zone again",
	Min = 30,
	Max = 300,
	Default = CONFIG.probeHoldSecs,
	Callback = function(v) CONFIG.probeHoldSecs = math.floor(v) end
})

-- Upgrades Tab
Tabs.Upgrades:Section({ Title = "AUTOMATION" })
Tabs.Upgrades:Toggle({
	Title = "Auto Buy Cutters",
	Desc = "DamageMult is the damage engine - first call on money",
	Default = CONFIG.cutters,
	Callback = function(v) CONFIG.cutters = v end
})
Tabs.Upgrades:Toggle({
	Title = "Auto Buy Auras",
	Desc = "Money priced, x1.2 up to x300 strength",
	Default = CONFIG.auras,
	Callback = function(v) CONFIG.auras = v end
})
Tabs.Upgrades:Toggle({
	Title = "Auto Buy Upgrades",
	Desc = "Attack speed, range, carry and walk speed",
	Default = CONFIG.upgrades,
	Callback = function(v) CONFIG.upgrades = v end
})
Tabs.Upgrades:Toggle({
	Title = "Auto Rebirth",
	Desc = "Gated on player level; unlocks training pads",
	Default = CONFIG.rebirth,
	Callback = function(v) CONFIG.rebirth = v end
})
Tabs.Upgrades:Toggle({
	Title = "Auto Travel Worlds",
	Desc = "Level gated: 185 / 380 / 570 / 750",
	Default = CONFIG.worlds,
	Callback = function(v) CONFIG.worlds = v end
})
Tabs.Upgrades:Toggle({
	Title = "Auto Free Rewards",
	Desc = "Offline, like reward, finished quests, free wheel spin",
	Default = CONFIG.freebies,
	Callback = function(v) CONFIG.freebies = v end
})
Tabs.Upgrades:Slider({
	Title = "Reserve Window (s)",
	Min = 15,
	Max = 300,
	Default = CONFIG.reserveWindow,
	Callback = function(v) CONFIG.reserveWindow = math.floor(v) end
})

Tabs.Upgrades:Section({ Title = "MANUAL ACTIONS" })
Tabs.Upgrades:Button({
	Title = "Trip Now",
	Desc = "Perform a manual cut pass",
	Callback = function() task.spawn(cutPass) end
})
Tabs.Upgrades:Button({
	Title = "Sell All Loot",
	Desc = "Sell backpack inventory now",
	Callback = function() task.spawn(sellPass) end
})
Tabs.Upgrades:Button({
	Title = "Shop Now",
	Desc = "Trigger cutter & aura purchase pass",
	Callback = function() task.spawn(function() cutterPass() auraPass() end) end
})
Tabs.Upgrades:Button({
	Title = "Claim Free Rewards",
	Desc = "Claim quests and timed rewards",
	Callback = function() task.spawn(freePass) end
})
Tabs.Upgrades:Button({
	Title = "Unstuck",
	Desc = "Reset movement state",
	Callback = unstuck
})

-- Status Tab
Tabs.Status:Section({ Title = "ANTI-CHEAT & SYSTEM" })
Tabs.Status:Label({
	Title = "Anti-Cheat Guard Active",
	Desc = "Never warps or edits WalkSpeed. Paced remote calls only."
})
Tabs.Status:Toggle({
	Title = "Heartbeat Anti-AFK",
	Default = true,
	Callback = function(v) CONFIG.antiAfk = v end
})

Tabs.Status:Section({ Title = "LIVE DASHBOARD" })
local statusLabel1 = Tabs.Status:Label({ Title = "Status: Initializing..." })
local statusLabel2 = Tabs.Status:Label({ Title = "Strength: -" })
local statusLabel3 = Tabs.Status:Label({ Title = "Money: -" })
local statusLabel4 = Tabs.Status:Label({ Title = "Level: - | Rebirths: - | World: -" })
local statusLabel5 = Tabs.Status:Label({ Title = "Zone: -" })
local statusLabel6 = Tabs.Status:Label({ Title = "Cutter: - | Aura: - | Pad: -" })
local statusLabel7 = Tabs.Status:Label({ Title = "Backpack: -" })
local statusLabel8 = Tabs.Status:Label({ Title = "Trips: - | Loot: -" })
local statusLabel9 = Tabs.Status:Label({ Title = "Earned: - | Spent: -" })
local statusLabel10 = Tabs.Status:Label({ Title = "Note: Ready" })

CONFIG.antiAfk = true
RunService.Heartbeat:Connect(function()
	if CONFIG.antiAfk and GEN == _G.__CUTGRASS then
		pcall(function()
...
task.spawn(function()
	while GEN == _G.__CUTGRASS do
		pcall(function()
			...
			local stText = STATE.halted and "HALTED BY ANTI-CHEAT WARNING" or (CONFIG.auto and "AUTO RUNNING (" .. tostring(STATE.phase) .. ")" or "STOPPED")
			statusLabel1:SetTitle("Status: " .. stText)
			statusLabel2:SetTitle(string.format("Strength: %s (+%s/s)", short(STATE.strength), short(STATE.strRate)))
			statusLabel3:SetTitle(string.format("Money: $%s (+$%s/s)", short(STATE.money), short(STATE.moneyRate)))
			statusLabel4:SetTitle(string.format("Lv: %s | Rebirths: %s | World: %s", tostring(STATE.level), tostring(STATE.rebirths), tostring(STATE.world)))
			statusLabel5:SetTitle(string.format("Zone: %s (Open to %s)", tostring(STATE.zone), tostring(STATE.zoneBest)))
			statusLabel6:SetTitle(string.format("Cutter: %s (x%s) | Aura: %s (x%s) | Pad: x%s", tostring(STATE.cutter), short(STATE.cutterMult), tostring(STATE.aura), tostring(STATE.auraMult), tostring(STATE.padMult)))
			statusLabel7:SetTitle(string.format("Backpack: %s/%s ($%s)", tostring(STATE.bag), tostring(STATE.bagCap), short(STATE.bagValue)))
			statusLabel8:SetTitle(string.format("Trips: %s (Walled: %s) | Loot: %s sold", tostring(STATE.trips), tostring(STATE.blocked), tostring(STATE.sold)))
			statusLabel9:SetTitle(string.format("Earned: $%s | Spent: $%s", short(STATE.earned), short(STATE.spent)))
			statusLabel10:SetTitle("Note: " .. tostring(STATE.note))
		end)
		task.wait(0.5)
	end
end)
...
print("[cutgrass] gen " .. GEN .. " ready with WindUI - IndraHub")
