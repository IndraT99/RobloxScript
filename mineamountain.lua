...
setGlobal("IndraHubMineAMountainRunning", true)
setGlobal("IndraHubMineAMountainLastHeartbeat", os.clock())
...
task.spawn(function()
	while true do
		task.wait(1)
		local now = os.clock()
		setGlobal("IndraHubMineAMountainLastHeartbeat", now)
		if _G.IndraHubStatus then
			if not _G.IndraHubStatus["IndraHubMineAMountainLastHeartbeat"] then
				_G.IndraHubStatus["IndraHubMineAMountainLastHeartbeat"] = {}
			end
			_G.IndraHubStatus["IndraHubMineAMountainLastHeartbeat"].heartbeat = now
		end
	end
end)

local Window = WindUI:CreateWindow({
	Title = "IndraHub | Mine a Mountain",
	Icon = "mountain",
	Author = "IndraHub",
	Folder = "IndraHub_MineAMountain",
...
local function updateBackpackLabel()
	...
	if capacity == math.huge then
		BackpackLabel:SetDesc(string.format("Bag %.1f / \u{221E} kg", used))
		return
	end

	local free = math.max(0, capacity - used)
	BackpackLabel:SetDesc(string.format("Bag %.1f / %.1f kg\nFree %.1f kg", used, capacity, free))
end
...
local function aimTeleport()
	if not espActive then
		Notify("Enable Crystal ESP first", 3)
		return
	end

	local inst = getAimedCrystal()
	if not inst then
		Notify("No crystal aimed", 2)
		return
	end

	if teleportTo(inst) then
		Notify(string.format("TP -> %s", crystalName(inst)), 2)
	else
...
local function tpToRank(scoreFn, rank, formatter)
	...
	if not entry or entry.score <= 0 then
		Notify(string.format("No crystal for #%d", rank), 3)
		return
	end
...
do
	local function install()
		Tabs.crystals:Section({ Title = "Crystal ESP" })

		Tabs.crystals:Toggle({
			Title = "Crystal ESP",
			Default = false,
			Callback = function(value)
				stateToggles["CrystalEsp"] = value
				espActive = value
				if not value then
					clearEsp()
				end
		...
		Tabs.crystals:Input({
			Title = "Min Value",
			Desc = "Min Value hides & skips crystals worth less than this (e.g. 500k, 2m, 1.5b)",
			Default = "2m",
			Placeholder = "2m",
...
do
	local function install()
		Tabs.players:Section({ Title = "Player ESP" })

		Tabs.players:Toggle({
			Title = "Player ESP",
			Default = false,
			Callback = function(value)
				stateToggles["PlayerEsp"] = value
...
do
	local function install()
		Tabs.boulders:Section({ Title = "Boulder ESP" })

		Tabs.boulders:Toggle({
			Title = "Boulder ESP",
			Default = false,
			Callback = function(value)
				stateToggles["BoulderEsp"] = value
		...
		Tabs.boulders:Divider()

		local infoLines = Mountain.boulderList()
		Tabs.boulders:Paragraph({
			Title = "Boulder Information",
			Desc = table.concat(infoLines, "\n"),
		})
	end
...
do
	local function install()
		Tabs.teleports:Section({ Title = "Top Crystals" })
		...
		for rank = 1, 3 do
			Tabs.teleports:Button({
				Title = string.format("Top %d Value", rank),
				Callback = function()
		...
		for rank = 1, 3 do
			Tabs.teleports:Button({
				Title = string.format("Top %d Luck", rank),
				Callback = function()
		...
		for rank = 1, 3 do
			Tabs.teleports:Button({
				Title = string.format("Top %d Weight", rank),
				Callback = function()
		...
		Tabs.teleports:Section({ Title = "Teleports & Actions" })

		Tabs.teleports:Toggle({
			Title = "Aim Teleport (Key: F)",
			Default = false,
			Callback = function(value)
				stateToggles["AimTeleport"] = value
				aimTpEnabled = value
			end,
		})
		...
		Tabs.teleports:Button({
			...
			Callback = function()
				if fireRemote(GoHome, "home") then
					Notify("Teleporting home", 2)
				else
					Notify("Remote unavailable", 2)
				end
			end,
		})
...
do
	local function install()
		Tabs.farming:Section({ Title = "Pickup Settings" })

		Tabs.farming:Toggle({
			Title = "Auto Pickup",
			Default = false,
			Callback = function(value)
				stateToggles["AutoPickup"] = value
		...
		Tabs.farming:Toggle({
			...
			Default = false,
			Callback = function(value)
				stateToggles["InstantPrompt"] = value
		...
		Tabs.farming:Toggle({
			...
			Default = false,
			Callback = function(value)
				stateToggles["AutoRunePickup"] = value
...
do
	local function install()
		Tabs.movement:Section({ Title = "Movement Mods" })

		Tabs.movement:Toggle({
			Title = "Speed Boost",
			Default = false,
			Callback = function(value)
				stateToggles["SpeedBoost"] = value
				setSpeedBoost(value)
			end,
		})
	...
	local function install()
		...
		local function step(deltaTime)
			...
			if phase == "reset" then
				...
				Notify("No boulders left - kicking and rejoining", 3)
				stop()
				restart()
			end
		end
		...
		local function setActive(value)
			...
			if not next(targets) then
				...
				stateToggles.AutoFarmBoulders = false
				return
			end
		...
		Tabs.boulders:Section({ Title = "Boulder Farm" })

		Tabs.boulders:Dropdown({
			Title = "Farm Targets",
			Values = FARM_KINDS,
			Multi = true,
			Default = {},
		...
		Tabs.boulders:Toggle({
			Title = "Auto Farm Boulders",
			Default = false,
			Callback = function(value)
				stateToggles["AutoFarmBoulders"] = value
				setActive(value)
			end,
		})
		...
		local StatusLabel = Tabs.boulders:Paragraph({
			Title = "Boulder Farm Status",
			Desc = "Idle",
		})
		local labelClock = 0
		...
		farmConn = RunService.Heartbeat:Connect(function(deltaTime)
			...
			if labelClock >= 0.25 then
				labelClock = 0
				StatusLabel:SetDesc(statusText)
			end
		end)
...
do
	local function install()
		...
		Tabs.farming:Section({ Title = "Money Farm" })

		Tabs.farming:Paragraph({
			Title = "Info",
		...
		Tabs.farming:Toggle({
			...
			Default = false,
			Callback = function(value)
				stateToggles["AutoFarmMoney"] = value
				setActive(value)
			end,
		})
		...
		Tabs.farming:Toggle({
			...
			Default = false,
			Callback = function(value)
				stateToggles["MoneyAutoSell"] = value
				autoSell = value
			end,
		})
		...
		local StatusLabel = Tabs.farming:Paragraph({
			Title = "Money Farm Status",
			Desc = "Idle",
		})
		local labelClock = 0
...
do
	local function install()
		...
		Tabs.settings:Paragraph({
			Title = "IndraHub | Mine a Mountain",
			Desc = "Script converted to WindUI with Supervisor Heartbeat integration.",
		})
